...
class ModuleFieldsHandler {
    ...
    */
    static async deleteFields(module) {
        try {
            let initializer = await initializer_1.Initializer.getInitializer();
            let recordFieldDetailsPath = path_1.default.join(initializer.getResourcePath(), constants_1.Constants.FIELD_DETAILS_DIRECTORY, await converter_1.Converter.getEncodedFileName());
            if (fs_1.default.existsSync(recordFieldDetailsPath)) {
