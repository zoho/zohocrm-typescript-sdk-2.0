...
class Utility {
    ...
    static async fileExistsFlow(moduleAPIName, recordFieldDetailsPath, lastModifiedTime) {
        let recordFieldDetailsJson = initializer_1.Initializer.getJSON(recordFieldDetailsPath);
    ...
    static async verifyModuleAPIName(moduleName) {
        ...
        if (moduleName != null && fs.existsSync(recordFieldDetailsPath)) {
            let fieldsJSON = initializer_1.Initializer.getJSON(recordFieldDetailsPath);
    ...
    */
    ...
    static async getModuleNames() {
        let moduleData = {};
        var resourcesPath = path.join((await initializer_1.Initializer.getInitializer()).getResourcePath(), constants_1.Constants.FIELD_DETAILS_DIRECTORY);
        if (!fs.existsSync(resourcesPath)) {
            fs.mkdirSync(resourcesPath, { recursive: true });
        }
    ...
    static async writeModuleMetaData(recordFieldDetailsPath, moduleData) {
        let fieldDetailsJSON = fs.existsSync(recordFieldDetailsPath) ? await initializer_1.Initializer.getJSON(recordFieldDetailsPath) : {};
